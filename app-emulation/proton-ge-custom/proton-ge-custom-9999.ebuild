...
src_install() {
	...
	doins -r "${PROJECT_NAME}"
}

pkg_postinst() {
	einfo "changing permission of proton executables"
	find  "${STEAM_COMPATDIR}/${PROJECT_NAME}/proton" -exec chmod ugo+x "{}" \;
	find  "${STEAM_COMPATDIR}/${PROJECT_NAME}/files/bin" -type f -exec chmod ugo+x "{}" \;
	find  "${STEAM_COMPATDIR}/${PROJECT_NAME}/protonfixes/winetricks" -exec chmod ugo+x "{}" \;
}
